    ...
    async loadCompanyCardData(company) {
        try {
            ...
            if (result.success && result.data) {
                ...
                // If still no vehicle data, estimate based on subscriber count
                if (totalVehicles === 0) {
                ...
                // Update subscriber count if we have it from enhanced API
                if (contractData.consumerCount) {
                    const subscribersEl = document.querySelector(`#companyList .company-card:nth-child(${Array.from(document.querySelectorAll('.company-card')).findIndex(c => c.onclick.toString().includes(company.id)) + 1}) .stat-value`);
                    if (subscribersEl) {
                ...
                if (facilityData) {
                    ...
                    if (presentEl) presentEl.textContent = presentValue.toString();
