        ...
        try {
            ...
            if (result.success && result.data) {
                ...
                if (finalSubscriberCount > 0) {
                    // Find the correct card by company ID
                    const cards = document.querySelectorAll('#companyList .company-card');
                    cards.forEach(card => {
                        const cardTitle = card.querySelector('.company-name');
                        if (cardTitle && cardTitle.textContent.includes(company.name)) {
                            const subscribersEl = card.querySelector('.stat-value');
                            if (subscribersEl) {
