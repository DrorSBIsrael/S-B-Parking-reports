...
def parking_manager_get_info():
    ...
    try:
        ...
        # בניית URL - נאפשר בחירה גמישה
        # עכשיו ננסה HTTPS עם הטיפול החדש ב-SSL
        force_http = False  # חזרנו ל-HTTPS עם טיפול מיוחד בתעודות
        ...
        # הסרנו את הבדיקה של IP פרטי - מאפשרים חיבור לכל IP
        # כי החניון יכול להיות ברשת פנימית
        
        # בדיקה מדויקת אם זה IP פרטי 
        # טווחים פרטיים: 10.x.x.x, 172.16-31.x.x, 192.168.x.x
        ip_address = parking_data.get('ip_address', '')
        try:
            is_private_ip = (
                ip_address.startswith('10.') or 
                ip_address.startswith('192.168.') or  # רק 192.168, לא כל 192!
                (ip_address.startswith('172.') and 
                 len(ip_address.split('.')) > 1 and
                 int(ip_address.split('.')[1]) >= 16 and 
                 int(ip_address.split('.')[1]) <= 31)
            )
        except:
            is_private_ip = False
        
        if is_private_ip:
            print(f"   ⚠️ Private IP detected ({ip_address}) - returning demo data")
            print(f"   💡 To connect to real parking, use Cloudflare Tunnel or ngrok")
        else:
            print(f"   ✅ Public IP detected ({ip_address}) - attempting real connection")
        
        # נחזיר נתוני דוגמה רק אם זה באמת IP פרטי
        if is_private_ip:
...
<response>
    ...
    <data></data>
        ...
        try:
            # timeout מוגבל ל-25 שניות
            timeout_seconds = 25
            print(f"   ⏱️ Attempting connection to {url}")
            ...
            print(f"⏱️ Timeout after {timeout_seconds}s connecting to {url}")
            print(f"   Parking IP: {ip_address}")
            print(f"   Possible causes: Firewall, wrong port, server down")
            return jsonify({'success': False, 'error': 'זמן ההמתנה לשרת החניון פג'}), 504
        except requests.exceptions.SSLError as e:
            print(f"🔐 SSL Error: {str(e)}")
            # במקרה של SSL, ננסה להחזיר נתוני דוגמה
            if is_private_ip:
                print("   Returning mock data due to SSL error")
                # כבר החזרנו mock data למעלה
            return jsonify({'success': False, 'error': 'בעיית אבטחה בחיבור'}), 503
        except requests.exceptions.ConnectionError as e:
            print(f"🔌 Connection error: {str(e)}")
            return jsonify({'success': False, 'error': 'לא ניתן להתחבר לשרת החניון'}), 503
        except Exception as e:
        ...
        print(f"❌ General proxy error: {str(e)}")
        print(f"   Error type: {type(e).__name__}")
        print(f"   Error occurred at proxy endpoint")
        print(f"   Request: {method} to {parking_id}/{endpoint}")
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'error': f'שגיאה במערכת: {str(e)}'}), 500
